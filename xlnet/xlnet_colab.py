...
from xpath import modeling


def _get_initializer(run_config):
  """Get variable intializer."""
  if run_config.init == "uniform":
    initializer = tf.initializers.random_uniform(
        minval=-run_config.init_range,
        maxval=run_config.init_range,
        seed=None)
  elif run_config.init == "normal":
    initializer = tf.initializers.random_normal(
        stddev=run_config.init_std,
        seed=None)
  else:
...
class XLNetConfig(object):
  ...
  def __init__(self, options=None, json_path=None):
    """Constructing an XLNetConfig.
    One of options dict or json_path should be provided."""

    assert options is not None or json_path is not None

    self.keys = ["n_layer", "d_model", "n_head", "d_head", "d_inner",
                 "ff_activation", "untie_r", "n_token"]

    if options is not None:
      self.init_from_flags(options)

    if json_path is not None:
      self.init_from_json(json_path)

  def init_from_flags(self, options):
  ...
  def to_json(self, json_path):
    ...
    tf.io.write_file(json_path, json.dumps(json_data, indent=4, sort_keys=True))

def create_run_config(is_training, is_finetune, options):
  kwargs = dict(
      is_training=is_training,
      use_tpu=options['use_tpu'],
      use_bfloat16=options['use_bfloat16'],
      dropout=options['hidden_dropout'],
      dropatt=options['attention_dropatt'],
      init=options['init'],
      init_range=options['init_range'],
      init_std=options['init_std'],
      clamp_len=options['clamp_len'])

  if not is_finetune:
    kwargs.update(dict(
        mem_len=options['mem_len'],
        reuse_len=options['reuse_len'],
        bi_data=options['bi_data'],
        clamp_len=options['clamp_len'],
        same_length=options['same_length']))
